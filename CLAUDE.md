...
│   ├── matches.csv                  # ブキ編成記録（自動生成・試合確定時に追記）
│   ├── results.csv                  # 試合勝敗記録（自動生成・/result 受信時に追記）
...
│   │   ├── csvWrite.ts              # RFC4180 CSV 出力ユーティリティ
│   │   ├── loadTeams.ts             # CSV → TeamsPool 変換
│   │   ├── weaponAliases.ts         # weapon_aliases.csv → WeaponAliases 変換
│   │   ├── appendWeapon.ts          # ブキ編成を matches.csv / Google Sheets に追記
│   │   ├── appendResult.ts          # 試合勝敗を results.csv / Google Sheets に追記
│   │   ├── screenshotWatcher.ts     # スクショ監視 → OCR トリガー
...
│       ├── selection.ts             # Selection（アルファ/ブラボー 選択状態）
│       ├── visibility.ts            # Visibility（アルファ/ブラボー 表示状態）
...
1. OCR 候補を Dashboard で確定 → `confirmMatchCandidate` メッセージ
2. Extension が `matches` Replicant に追加
3. `appendWeaponCsv` で `data/matches.csv` に alpha/bravo 各 1 行追記
4. `googleSheetSync` が有効なら `appendWeaponGoogleSheet` で Google Sheets にも追記

### 試合記録フロー（勝敗）

1. OBS 等の外部ツールが `POST /result` に `alpha_win` または `bravo_win` を送信
2. Extension が現在の `activeMode` と `selection` を取得し `appendResultCsv` で `data/results.csv` に追記
3. `googleSheetSync` が有効なら `appendResultGoogleSheet` で Google Sheets にも追記
...
- **CSV 出力**: `appendWeapon.ts`（ブキ編成）と `appendResult.ts`（勝敗）が別々のトリガーで `data/` に追記する。`csvWrite.ts` の `serializeRow` を共通ユーティリティとして使用。
- **Google Sheets 同期**: `googleSheetSync` Replicant が true かつ `.env` の GAS エンドポイント URL が設定されている場合のみ動作。失敗はログ出力のみで処理は続行。
