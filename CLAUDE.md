...
4. Dashboard で「表示」ボタン → `showTeam` メッセージ → `visibility` 更新 → Graphic がフェードイン
...
> [!NOTE]
> `new-panel.md` と `new-graphic.md` は旧プロジェクト固有のパターンを参照している。現プロジェクトのパネルを参考にする場合は `_shared/` 内の共通コンポーネントを参照すること。

## 参考リソース

- 公式 README: [README.md](README.md)
