...
### `team.id` と `team.name` の使い分け

| フィールド | 用途 |
|---|---|
| `team.id` | CSV 原本のチーム名。変更不可の一意キー。**Graphic 以外ではこちらを使う**（records.csv・Google Sheets 出力・Dashboard 表示・OCR 候補など） |
| `team.name` | Dashboard で自由編集できる表示名。HTML タグ（`<br>` 等）を含む場合がある。**Graphic（配信オーバーレイ）でのみ使う** |

`team.id === team.name`（CSV 読み込み時点では同値）だが、運営が `name` を編集すると乖離する。
Graphic 以外のコードで `team.name` を参照していた場合は `team.id` に修正すること。
...
│   │   ├── appendRecord.ts          # ブキ編成+勝敗を records.csv / Google Sheets に統合追記
...
│       ├── stageNames.ts            # StageNames 型（モード別ステージ名配列）
│       ├── weaponAliases.ts         # WeaponAliases 型
│       ├── googleSheetSync.ts       # GoogleSheetSync / GasEndpointConfigured 型
│       └── inGameNames.ts           # InGameNames 型
├── gas/
